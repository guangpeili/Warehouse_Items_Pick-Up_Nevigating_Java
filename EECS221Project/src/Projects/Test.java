...
public class Test {
	...
	public static void main(String[] args) {
		...
		//Retrieve order information about its contained items
				...
				//Intrigue user to input starting and ending position
				...
				//Let user determine which algorithm to use
				// 1 for nearest neighbor, and 2 for branch and bound
				System.out.println("Please choose the searching algorithm");
				System.out.println("Enter 1 to choose nearest neighbor");
				System.out.println("Enter 2 to choose branch and bound");
				Scanner ina = new Scanner(System.in);
				int algo = ina.nextInt();
				...
				if (algo == 1) {
					...
					//Do nearest neighbor on all items in the order
					while (true) {
						...
						for (int i = 0; i < numItem; i++) {
							if (dist[indexNow][i] < comp && dist[indexNow][i] != -1 && !picked.contains(stack.get(i))) {
								indexNext = i;
								comp = dist[indexNow][i];
							}
						}
					...
					System.out.format("The total distance of the generated path is: %d", totalDist);
					System.out.println();
					System.out.println();
					
					//Display total cost
					if (complete) {
						System.out.format("The total effort of the generated path is: %.2f", totalCost);
						System.out.println();
					} else {
						System.out.println("The weight information of involved item(s) is not complete, total effort cannot be generated");
						System.out.println();
					}
					
					
					String[] ss = addArrow(removeDuplicate(completePath));
					System.out.println();
					int ssn = ss.length;
						...
						if (temp < minDist) {
							...
							while (true) {
								...
								if (elapsedTime > 300000) {
									System.out.println("Exceeds 30s time limit, result may be awful :(");
									break;
								}
	...
	//Convert all Strings to Doubles
		...
		//Add arrow 2
		public static String[] addArrow(ArrayList<int[]> list) {
			
			int n = list.size();
			
			String[] ss = new String[n + 1];
			
			ss[0] = "The path would be:  ";
